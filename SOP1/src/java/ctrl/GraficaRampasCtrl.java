...
import org.zkoss.zul.Div;
  ...
  public class GraficaRampasCtrl extends GenericForwardComposer {
    ...
    public void Grafica1(String Fecha_inicial, String Fecha_final) throws SQLException {
        GraficaRampasDal dataBase = new GraficaRampasDal();

        grafica1.setType("column");
        grafica1.setTitle("Contenedores por Rampas");
        ...
        List<GraficaRampasMd> data = dataBase.Grafica1(Fecha_inicial, Fecha_final);
        String categoria = "";

        int i = 0;
        
        for (GraficaRampasMd valor : data) {
            if (categoria.equals("")) {
                categoria = "'" + valor.getRampas()+ "'";
            } else {
                categoria = categoria + "," + "'" + valor.getRampas()+ "'";
            }
            model.addValue("Total por Rampas", i, Integer.parseInt(valor.getSumarampas()));
              i++;
        }
    ...
    public void Grafica2(String fecha_inicial, String fecha_final) throws SQLException {
        ...
        grafica2.setTitle("PORCENTAJE CONTENEDORES POR RAMPAS");
