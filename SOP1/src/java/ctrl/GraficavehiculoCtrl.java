...
public class GraficavehiculoCtrl extends GenericForwardComposer {
    ...
    public void Grafica1(String Fecha_inicial, String Fecha_final) throws SQLException {
        GraficavehiculoDal dataBase = new GraficavehiculoDal();

        grafica1.setType("column");
        grafica1.setTitle("Vehiculos Nuevos y Usados");
        ...
        List<GraficavehiculoMd> data = dataBase.Grafica1(Fecha_inicial, Fecha_final);
        String categoria = "";

        int i = 0;

        for (GraficavehiculoMd valor : data) {
            if (categoria.equals("")) {
                categoria = "'" + valor.getVehiculo() + "'";
            } else {
                categoria = categoria + "," + "'" + valor.getVehiculo() + "'";
            }
            model.addValue("Total Vehiculos Nuevos y Usados", i, Integer.parseInt(valor.getSumavehiculo()));
            i++;
        }
    ...
    public void Grafica2(String fecha_inicial, String fecha_final) throws SQLException {
        ...
        grafica2.setTitle("PORCENTAJE DE VEHICULOS NUEVOS/USADOS");
