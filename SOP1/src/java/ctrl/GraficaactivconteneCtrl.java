...
import DAL.GraficaactivconteneDal;
...
public class GraficaactivconteneCtrl extends GenericForwardComposer {
  ...
  //  private Datebox Fecha_Inicial;
    ...
    public void onChange$Fecha_Final(Event evn) throws SQLException {
        ...
        Grafica1(Fecha_Final.getText());
        
         Clients.showNotification("NO SE GUARDO EL REGISTRO DEBE LLENAR LOS CAMPOS VACIOS!  <br/> INGRESE FECHA POR FAVOR PARA GENERAR LA GRAFICA....!!!!<br/>",
                    "warning", null, "middle_center", 5000);
    }

    //GRAFICA #1 en DAL
    public void Grafica1(String Fecha_final) throws SQLException {
        GraficaactivconteneDal dataBase = new GraficaactivconteneDal();

        grafica1.setType("column");
        grafica1.setTitle("Actividades de Contenedores");
        //  grafica1.setSubTitle("Fechas de Atraque "+Fecha_inicial +'-'+Fecha_final);
        ...
        List<GraficaactivconteneMd> data = dataBase.Grafica1(Fecha_final);
        String categoria = "";

        int i = 0;

        for (GraficaactivconteneMd valor : data) {
            if (categoria.equals("")) {
                categoria = "'" + valor.getActividad() + "'";
            } else {
                categoria = categoria + "," + "'" + valor.getActividad() + "'";
            }
            model.addValue("Total actividades..!!!!", i, Integer.parseInt(valor.getSumaactividad()));
            i++;
        }
    ...
    //GRAFICA #2 en DAL
    public void Grafica2(String fecha_final) throws SQLException {
        GraficaactivconteneDal dataBase = new GraficaactivconteneDal();

        grafica2.setTitle("PORCENTAJE DE ACTIVIDADES DE BUQUE <BR>"
                + "IMPORTACION, EXPORTACION Y RAYOS X");
        ...
        grafica2.setPlotOptions("{"
                ...
                + "}"
                + "}");
        grafica2.setModel(dataBase.Grafica2(fecha_final));
    }
